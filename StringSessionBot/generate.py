...
async def generate_session(bot, msg, telethon=False):
    await msg.reply(
        f'Seccusfully {"Telethon" if telethon else "Pyrogram"} Session Generation...'
    )
    ...
    text = f'**{"TELETHON" if telethon else "PYROGRAM"} STRING SESSION** \n\n`{string_session}` \n\nGenerated by @TheStringSessionBot'

    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply(
        f'Seccusfully {"telethon" if telethon else "pyrogram"} string session. \n\nNow Check/Saved Message! \n\nBy @TheStringSessionBot'
    )
